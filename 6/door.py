...
message_server = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
message_server.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
message_server.settimeout(0.2)
message_server.bind(("", 44444))
message = b"Your beautiful"
def ring():
    print ("Sent message")
    message_server.sendto(message, ('<broadcast>', 42069))
button = Button(22)
...
background-message_size: 1800% 1800%;
